...
restaurant_df = pd.read_csv('data/JEJU_PROCESSED.csv', usecols=['MCT_NM', 'LATITUDE', 'LONGITUDE'], encoding='cp949')
...
restaurant_df = restaurant_df.rename(columns={'MCT_NM': 'PLACE_NAME', 'LATITUDE': 'LATITUDE', 'LONGITUDE': 'LONGITUDE'})

# Load tourist destination data from JEJU_TOUR.csv
tourist_df = pd.read_csv('data/JEJU_TOUR.csv', usecols=['TOUR_NM', 'Latitude', 'Longitude'], encoding='cp949')

# Rename columns to match the required output
tourist_df = tourist_df.rename(columns={'TOUR_NM': 'PLACE_NAME', 'Latitude': 'LATITUDE', 'Longitude': 'LONGITUDE'})
