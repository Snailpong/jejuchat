...
# Load the data
blue_data = pd.read_csv('data/blueribbon.csv')  # Contains NAME, LATITUDE, LONGITUDE, ADDRESS
mct_data = pd.read_csv('data/JEJU_PROCESSED.csv', encoding='cp949')  # Contains MCT_NM, LATITUDE, LONGITUDE
...
for blue_idx, blue_row in tqdm(blue_data.iterrows(), total=len(blue_data)):
    ...
    if len(exact_matches) == 1:
        ...
        matches.append({
            'MCT_NAME': exact_matches.iloc[0]['MCT_NM'], 
            'BLUE_NAME': blue_row['NAME']
        })
    else:
        # Step 2: Find partial matches (using cleaned names)
        partial_matches = mct_data[mct_data['CLEANED_MCT_NM'].str.contains(blue_name, na=False)]
        ...
        if not partial_matches.empty:
            ...
            if len(valid_matches) == 1:
                matches.append({'MCT_NAME': valid_matches[0]['MCT_NAME'], 'BLUE_NAME': blue_row['NAME']})
